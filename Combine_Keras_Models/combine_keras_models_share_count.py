...
class Static:
    ...
    group_size = limit
    plot_losses = PlotLearning("combined_keras_model_Share_count")
    batch_size = 1
    epochs = 50
...
def get_data():
    ...
    Share_counts = list(map(lambda x: x[3], data))
...
for epoch in range(Static.epochs):
    newModel.fit([np.array(trainX_image), np.squeeze(np.array(trainX_message), axis=1)],
                 ...
                 callbacks=[Static.plot_losses], batch_size=Static.batch_size, epochs=1)
