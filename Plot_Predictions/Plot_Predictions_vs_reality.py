...
def get_data():
    ...
    y_data = list(map(lambda x: np.log(x) if x > 0 else 0, map(lambda x: Static.metric_getter(x), ids)))
...
for trainX_image, trainX_message, trainY, testX_image, testX_message, testY in get_data():
    model = load_model("../Combine_Keras_Models/Combined_Model.h5")
    ...
    for pred, real in zip(flatten_predictions, testY):
        print(pred, real)
