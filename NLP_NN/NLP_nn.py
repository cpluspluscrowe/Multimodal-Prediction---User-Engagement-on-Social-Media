...
if "Share" in Global.metric_getter.__name__:
    ...
    Global.regularizer_function = np.log
    ...
    Global.regularizer_function = identity
else:
...
y_data = list(map(lambda x: Global.regularizer_function(x) if x > 0 else 0, df[Global.metric_name]))
print(len(x_data))
print(len(y_data))
...
def data_generator(metricGetter):
    counts = np.array(ApplyMinMaxScalar(y_data))
